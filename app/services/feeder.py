...
class FeederService:
    ...
    async def back_sync_badges(self, intake: BackSyncIntakeSchema):
        async with BackgroungInterlock('back_sync_badges'):
    ...
    async def back_sync_arrivals(self, intake: BackSyncIntakeSchema):
        async with BackgroungInterlock('back_sync_arrivals'):
    ...
    async def sync(self, from_date: datetime):
        if os.path.isfile('__tasks__/back_sync_badges') or os.path.isfile('__tasks__/back_sync_arrivals') or os.path.isfile('__tasks__/back_sync'):
            raise WaitForItError()
